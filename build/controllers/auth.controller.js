...
const createSession = (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    ...
    try {
        ...
        logger_1.logger.error('ERR: auth - create session = ', error.message);
        return res.status(422).send({
            status: false,
            statusCode: 422,
...
const refreshSession = (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    ...
    if (error) {
        logger_1.logger.error('ERR: auth - refersh session = ', error.details[0].message);
        return res.status(422).send({
            status: false,
            statusCode: 422,
            message: error.details[0].message
    ...
    try {
        ...
        return res.status(200).send({
            status: false,
            statusCode: 200,
            message: 'Refreshh Success',
            data: { accessToken }
        });
    }
    catch (error) {
        logger_1.logger.error('ERR: auth - refresh session = ', error.message);
        return res.status(422).send({
            status: false,
            statusCode: 422,
            message: error.message
